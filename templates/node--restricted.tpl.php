<div id="node-<?php print $node->nid; ?>" class="basic-content <?php print $classes; ?>">
	...
	<?php
	$sidebarCount = 0;
	if (isset($field_restricted_sidebar_image)) {
    	foreach ($field_restricted_sidebar_image as $entry) {  
    ...
    <?php } ?>
  	...
  	<div class="content <?php if (jrmckenzie_show_sidebar($node)) { ?>content-with-sidebar<?php } ?>">
		...
		if (isset($field_restricted_full_image)) { ?>
		...
		<?php
    		foreach ($field_restricted_full_image as $entry) {    	
